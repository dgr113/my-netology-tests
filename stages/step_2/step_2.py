...
class Network(Module):
    def __init__(self):
        super().__init__()
        self.ff_1 = Flatten()
        self.ff_2 = Linear(in_features=784, out_features=512)
        self.ff_3 = Linear(in_features=512, out_features=256)
        self.out = Linear(in_features=256, out_features=10)

    def forward(self, out: 'Tensor') -> 'Tensor':
        out = relu(self.ff_1(out))
        out = relu(self.ff_2(out))
        out = relu(self.ff_3(out))
        out = self.out(out)
        return out
